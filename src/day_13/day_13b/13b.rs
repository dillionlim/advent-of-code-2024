// Implementation for Day 13, Part B
use crate::common::get_input;
use std::path::Path;

fn process_input(input: Vec<String>) -> Vec<((i64, i64), (i64, i64), (i64, i64))> {
    ...
    for line in input {
        if line.starts_with("Button A:") {
            ...
            let x = parts[0]["X+".len()..].parse::<i64>().unwrap();
            let y = parts[1]["Y+".len()..].parse::<i64>().unwrap();
            current_tuple.0 = (x, y);
        } 
...
fn process_testcase(button_a: (i64, i64), button_b: (i64, i64), prize: (i64, i64)) -> Option<i64> {
...
pub fn solve() -> String {
    ...
    for (button_a, button_b, prize) in processed_input {
        ...
        if let Some(result) = process_testcase(button_a, button_b, new_prize) {
            ans += result;
        }
    }
