// Implementation for Day 21, Part B
...
fn find_shortest_paths_numpad(character: &u8, starting_char: &u8) -> Vec<Vec<usize>> {
    ...
    while !queue.is_empty() {
        ...
        for _ in 0..sz {
            ...
            if curr == *character as usize {
                ...
                backtracking.push_back((*character, 0usize));
                while let Some((curr, prev_dirs)) = backtracking.pop_front() {
                    ...
                    for &(parent, dir) in parents[curr as usize].iter() {
                        let curr_dir = prev_dirs * 10 + dir as usize;
...
fn hash_pos_robots(curr: u8, next: u8, robots: u8) -> usize {
    (curr as usize) | ((next as usize) << 8) | ((robots as usize) << 16)
}

fn find_shortest_len_rec(curr: u8, next: u8, robots: u8, dp: &mut HashMap<usize, usize>) -> usize {
    ...
    while !queue.is_empty() {
        ...
        for _ in 0..sz {
            ...
            if curr_pos == next {
                let mut backtracking = VecDeque::new();
                backtracking.push_back((next, 0usize));
...
fn find_shortest_len_dirpad(directions: &Vec<usize>, robots: u8, dp: &mut HashMap<usize, usize>) -> usize {
    if robots == 0 {
        return directions.len() as usize;
    }
    let mut curr = 4;
    let mut tot_len = 0;
...
pub fn solve() -> String {
    ...
    for code in codes {
        let numeric_part = std::str::from_utf8(&code[0..=2]).unwrap().parse::<usize>().unwrap();
        ...
        for character in code.iter() {
            ...
            tot_len_of_path += shortest_paths.iter().map(|path| find_shortest_len_dirpad(path, 25, &mut dp)).min().unwrap();
            prev = curr;
        }
